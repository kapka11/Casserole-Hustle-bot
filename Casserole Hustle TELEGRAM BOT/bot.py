...
async def cmd_buy(upd, ctx):
    ...
    buyer = get_user(upd.effective_user.id, upd.effective_chat.id, upd.effective_user.username or "", upd.effective_user.first_name or "")
    seller = get_user(s.id, upd.effective_chat.id, s.username or "", s.first_name or "")
    ...
    sget = cost - comm
    upd_user(upd.effective_user.id, upd.effective_chat.id, balance=buyer["balance"]-cost, casseroles=buyer["casseroles"]+amt)
    upd_user(s.id, upd.effective_chat.id, balance=seller["balance"]+sget, casseroles=seller["casseroles"]-amt)
    await upd.message.reply_text(f"✅ Куплено {amt} запеканок за {cost}. Продавец получил {sget} (комиссия: {comm})")
...
async def cmd_buy_s(upd, ctx):
    ...
    comm = int(cost * COMMISSION)
    sget = cost - comm
    upd_user(upd.effective_user.id, upd.effective_chat.id, balance=buyer["balance"]-cost, syrniki=buyer["syrniki"]+amt)
    upd_user(s.id, upd.effective_chat.id, balance=seller["balance"]+sget, syrniki=seller["syrniki"]-amt)
    await upd.message.reply_text(f"✅ Куплено {amt} сырников за {cost}. Продавец получил {sget} (комиссия: {comm})")
...
async def cmd_sell(upd, ctx):
    ...
    if amt <= 0: return await upd.message.reply_text("Положительное число!")
    cost = int(amt * C_PRICE)
    seller = get_user(upd.effective_user.id, upd.effective_chat.id, upd.effective_user.username or "", upd.effective_user.first_name or "")
    buyer = get_user(b.id, upd.effective_chat.id, b.username or "", b.first_name or "")
    ...
    comm = int(cost * COMMISSION)
    sget = cost - comm
    upd_user(upd.effective_user.id, upd.effective_chat.id, casseroles=seller["casseroles"]-amt, balance=seller["balance"]+sget)
    upd_user(b.id, upd.effective_chat.id, balance=buyer["balance"]-cost, casseroles=buyer["casseroles"]+amt)
    await upd.message.reply_text(f"✅ Продано {amt} запеканок за {cost}. Получено {sget} (комиссия: {comm})")
...
async def cmd_sell_s(upd, ctx):
    ...
    comm = int(cost * COMMISSION)
    sget = cost - comm
    upd_user(upd.effective_user.id, upd.effective_chat.id, syrniki=seller["syrniki"]-amt, balance=seller["balance"]+sget)
    upd_user(b.id, upd.effective_chat.id, balance=buyer["balance"]-cost, syrniki=buyer["syrniki"]+amt)
    await upd.message.reply_text(f"✅ Продано {amt} сырников за {cost}. Получено {sget} (комиссия: {comm})")
